...
*/
        ...
        public void RenewToken()
        {
            ...
            {
                throw new Exception("Failed to renew Spotify token", exc);
            }
        }
        ...
        public string GetArt(string uri)
        {
            ...
            catch (WebException webException)
            {
                if (webException.Response != null && ((HttpWebResponse)webException.Response).StatusCode != HttpStatusCode.NotFound)
                    throw new Exception("[WebException] Failed to retrieve cover url from Spotify", webException);
            }
            catch (Exception exc)
            {
                throw new Exception("Failed to retrieve cover url from Spotify", exc);
            }
            return string.Empty;
        }
        ...
        public Status Status
        {
            get
            {
                ...
                {
                    throw new Exception("Failed to get track status", exc);
                    return null;
                }
            }
        }
        ...
        private string GetOAuth()
        {
            ...
            oClient.Headers.Add(HttpRequestHeader.UserAgent, "Mozilla/5.0 (Windows NT x.y; Win64; x64; rv:10.0) Gecko/20100101 Firefox/10.0");
            string cContent = oClient.DownloadString("https://embed.spotify.com/?uri=spotify:track:5Zp4SWOpbuOdnsxLqwgutt");

            List<string> oLstLines = new List<string>();


            string[] aLines = cContent
                                     .Replace(" ", "")
                                     .Split(new[] { "\n" }, StringSplitOptions.RemoveEmptyEntries);


            oLstLines = aLines.ToList();

            string cLineSpotifyToken = oLstLines.Find(x => x.Contains("tokenData")).Split('\'')[1];

            return cLineSpotifyToken;
        ...
        private string SendLocalRequest(string request, bool oauth, bool cfid, int wait)
        {
            ...
            var requestUri = "http://localhost:4380/" + request + parameters;
            ...
            {
                throw new Exception("SendLocalRequest failed", wExc);
                ...
                if (Process.GetProcessesByName("SpotifyWebHelper").Length < 1)
                {
                    ...
                    {
                        throw new Exception("Failed to start the Spotify webhelper", exc);
